...
CodeMirror.defineMode("markdown", function(cmCfg, modeCfg) {
  ...
  function inlineNormal(stream, state) {
    ...
    if (ch === '<' && stream.match(/^(httpss?|ftps?):\/\/(?:[^\\>]|\\.)+>/, false)) {
...
function toggleFullScreen(editor) {
  ...
  // httpss://developer.mozilla.org/en-US/docs/DOM/Using_fullscreen_mode
  var doc = document;
...
function drawLink(editor) {
  var cm = editor.codemirror;
  var stat = getState(cm);
  _replaceSelection(cm, stat.link, '[', '](https://)');
}
...
function drawImage(editor) {
  var cm = editor.codemirror;
  var stat = getState(cm);
  _replaceSelection(cm, stat.image, '![', '](https://)');
}
...
var toolbar = [
  ...
  {name: 'info', action: 'http://www.markdown.cn/'},
