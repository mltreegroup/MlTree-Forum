...
class Topic extends Model
{
    ...
    public function TopicList($page, $type, $fid = 0)
    {
        ...
        switch ($type) {
            case 'tops':
                if ($fid == 0) {
                    $topicData = Topic::page('topic')->where('tops', 1)->page($page, $max)->order(['tid'=>'desc','create_time'=>'desc'])->select();
                } else {
                    $topicData = Topic::page('topic')->where('tops', 1)->where('fid', $fid)->page($page, $max)->order(['tid','create_time'=>'desc'])->select();
                }
                $count = Topic::page('topic')->where('tops', 1)->count('tid');
                $pages = ceil($count / $max);
                break;
            case 'essence':
                if ($fid == 0) {
                    $topicData = Topic::page('topic')->where('essence', 1)->page($page, $max)->order(['tid'=>'desc','create_time'=>'desc'])->select();
                } else {
                    $topicData = Topic::page('topic')->where('essence', 1)->where('fid', $fid)->page($page, $max)->order(['tid'=>'desc','create_time'=>'desc'])->select();
                }
            ...
            default:
                if ($fid == 0) {
                    $topicData = Topic::page($page, $max)->order(['tid'=>'desc','create_time'=>'desc'])->select();
                } else {
                    $topicData = Topic::page($page, $max)->where('fid', $fid)->order(['tid'=>'desc','create_time'=>'desc'])->select();
                }
